...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: currentUser == null 
          ? _buildNotLoggedIn(authService) 
          : _buildProfile(currentUser, authService),
    );
  }

  Widget _buildNotLoggedIn(AuthService authService) {
    return Center(
      child: Column(
  ...
  Widget _buildProfile(AppUser currentUser, AuthService authService) {
    return SingleChildScrollView(
      padding: EdgeInsets.all(16),
      child: Column(
        children: [
          _buildUserCard(currentUser, authService),
          SizedBox(height: 20),
          _buildAddressesCard(),
          SizedBox(height: 20),
          ...
          SizedBox(height: 20),
          _buildSettingsCard(authService),
          SizedBox(height: 40),
        ],
      ),
    );
  ...
  Widget _buildUserCard(AppUser user, AuthService authService) {
    return Card(
  ...
  Widget _buildSettingsCard(AuthService authService) {
    return Card(
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(16),
      ),
  ...
  void _logout(AuthService authService) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(
